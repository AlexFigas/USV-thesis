...
Neste capítulo é apresentada a implementação do sistema ciberfísico desenvolvido, abordando de forma integrada as suas componentes de \emph{hardware} e \emph{software}. Esta análise encontra-se organizada em quatro secções principais, permitindo compreender a forma como o sistema foi concebido, desenvolvido e validado.  

Na Secção~\ref{sec:plataforma-desenvolvimento} descreve-se a plataforma de desenvolvimento, onde são analisadas as ferramentas e tecnologias utilizadas tanto a nível de microcontrolador como do ambiente de programação. A Secção~\ref{sec:implementacao-hardware} aborda a implementação de \emph{hardware}, apresentando a integração dos propulsores, sensores, módulos de comunicação e a conceção da \gls{pcb}. Na Secção~\ref{sec:implementacao-software} é detalhada a implementação de \emph{software}, incluindo a arquitetura modular, o controlo dos motores, o processamento de dados dos sensores e a comunicação via \gls{lora}. Por fim, a Secção~\ref{sec:implementacao-sumario} apresenta um sumário conclusivo, sintetizando os principais resultados e destacando as decisões de projeto que asseguram a robustez, escalabilidade e reprodutibilidade do sistema.

Esta organização permite uma leitura estruturada e progressiva, conduzindo o leitor do contexto geral de desenvolvimento até aos detalhes técnicos da implementação que materializam o sistema proposto.
...
Embora o desenvolvimento principal tenha sido efetuado no Visual Studio Code com o suporte do PlatformIO, foram também realizados testes complementares no ambiente Arduino IDE, devido à sua simplicidade na programação inicial do \gls{esp32} e na verificação rápida da compatibilidade das bibliotecas utilizadas.
...
A implementação do \emph{hardware} do sistema desenvolvido neste \gls{tfm} assenta numa arquitetura modular, representada na forma de esquema elétrico no Anexo \ref{anexo:pcb}. Tal como no projeto realizado em \cite{didactic-robot-thesis}, procurou-se manter uma estrutura clara e escalável, onde cada componente desempenha um papel bem definido e comunica com a unidade central de forma eficiente.
...
A alimentação do módulo é assegurada diretamente pela linha de 5 V do sistema, enquanto as ligações de dados (TX e RX) passam pelo conversor de nível lógico antes de chegar ao \gls{esp32}. Esta configuração garante a integridade do sinal e a proteção dos módulos. As ligações elétricas detalhadas podem ser observadas no Anexo \ref{anexo:pcb}, na Figura \ref{fig:esquematica-gps}, que apresenta o esquema elétrico correspondente.
...
A Figura \ref{fig:pcb-esquematica-vs-real} ilustra a comparação entre o esquema elétrico desenvolvida e a implementação física da placa.
...
\begin{figure}[H]
    ...
    \begin{minipage}{0.45\linewidth}
        ...
        \caption*{(A) \emph{Layout} da \gls{pcb}}
    \end{minipage}
    \hfill
...
\begin{lstlisting}[style={BetterCPP},caption={Comparação entre representação textual e binária com \texttt{Protobuf}},label={lst:comp}]
-- String (JSON-like):
...
Apesar de simples, este método revelou-se suficiente para o protótipo descrito neste \gls{tfm}, assegurando um comportamento previsível e estável em todas as fases de teste. A arquitetura modular permite ainda a substituição futura desta lógica por controladores mais avançados, como \acrfull{pid} ou \acrfull{lqr}, sem impacto significativo no restante sistema.  
...
\lstinputlisting[caption=Métodos \texttt{setDirection()} e \texttt{pulseToTicks()} da classe \texttt{Thruster}, label={lst:thruster-cpp}, style=BetterCPP]{code/minimal/thruster.cpp}
...
\subsection{Comunicação \acrfull{lora}}
...
Em síntese, a integração da comunicação \gls{lora} no TTGO \gls{lora}32, em conjunto com a abstração fornecida pelas classes \texttt{LoRaDuplex} e \texttt{LoRaProto}, resultou numa solução eficiente, escalável e robusta, permitindo não apenas o envio de telemetria em tempo real, mas também a receção de comandos de controlo com latência minima.
...
Ao nível do \emph{software}, a arquitetura foi organizada em pacotes independentes, promovendo o encapsulamento de funcionalidades e a reutilização de código. Foram desenvolvidos módulos próprios para o controlo dos motores, aquisição e processamento de dados de sensores, comunicação sem fios via \gls{lora} e navegação autónoma baseada em \emph{waypoints}. Esta modularidade garante não só a clareza e robustez do sistema, mas também a sua capacidade de evolução, permitindo a integração de novos sensores, algoritmos de controlo mais avançados ou protocolos de comunicação alternativos.  
