...
\begin{itemize}
    ...
    \item Em caso de perda de sinal, o \gls{esc} entra em \emph{safe mode} e bloqueia o motor para evitar movimentos inesperados.
\end{itemize}
...
A Figura~\ref{fig:imu-axes-equations} apresenta uma representação tridimensional dos eixos do sensor \gls{imu} e dos ângulos de orientação (\emph{yaw}, \emph{pitch} e \emph{roll}) obtidos a partir das medições dos seus três subsistemas principais: o acelerómetro, o giroscópio e o magnetómetro.

\begin{figure}[H]
    \centering
    \includegraphics[width=1\linewidth]{figuras/imu_orientation_plot.png}
    \caption[Representação tridimensional dos eixos do \gls{imu} e ângulos de orientação]{Representação tridimensional dos eixos do \gls{imu} e dos ângulos de orientação \emph{yaw} (\(\Psi\)), \emph{pitch} (\(\Theta\)) e \emph{roll} (\(\Phi\)). São ainda mostrados os vetores de aceleração \(\vec{a} = (a_x, a_y, a_z)\) e de campo magnético \(\vec{m} = (m_x, m_y, m_z)\), que servem de base para o cálculo das equações apresentadas de seguida.}
    \label{fig:imu-axes-equations}
\end{figure}

Estes sensores medem, respetivamente, a aceleração linear, a velocidade angular e a intensidade do campo magnético terrestre em torno dos três eixos ortogonais \(x\), \(y\) e \(z\).
...
O protocolo \gls{i2c} é um dos mais comuns para comunicação entre dispositivos periféricos em sistemas embebidos, como o \gls{usv}. A sua principal vantagem reside na capacidade de utilizar apenas dois fios para comunicação: um para o relógio (\(SCL\)) e outro para dados (\(SDA\)), o que reduz significativamente a complexidade e o número de interligações no sistema. Ao contrário do \gls{spi}, que normalmente requer uma linha de seleção de dispositivo (\emph{chip select}) por cada periférico, o \gls{i2c} permite que múltiplos dispositivos sejam conectados ao mesmo barramento utilizando apenas estas duas linhas, o que simplifica o projeto de \emph{hardware} e facilita a expansão do sistema com novos sensores ou atuadores. Cada dispositivo é identificado por um endereço único, permitindo que um único controlador mestre se comunique com vários escravos de forma organizada e padronizada.

A utilização do \gls{i2c} neste projeto é fundamental para a integração de sensores e módulos que requerem comunicação de dados eficiente e com baixo consumo energético. A flexibilidade do \gls{i2c} permite expandir facilmente o número de dispositivos conectados ao sistema, com a adição de novos expansores de I/O, como descrito em \cite{didactic-robot-thesis} Secção 4.4, que podem ser utilizados para controlar sensores adicionais ou atuadores, como motores e atuadores de direção. O protocolo \gls{i2c} é amplamente utilizado em módulos de sensores como o MPU9250 (\gls{imu}), que requerem comunicação constante para garantir medições em tempo real da orientação e movimento do \gls{usv}. Além disso, o \gls{i2c}, por usar apenas duas linhas e possuir uma taxa de transmissão moderada, apresenta menor consumo energético e menor complexidade de cablagem comparado ao \gls{spi}, sendo ideal para sistemas compactos e de baixo consumo.
