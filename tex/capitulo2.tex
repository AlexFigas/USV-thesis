% !TeX spellcheck = pt_PT2

\chapter{Estado da Arte} \label{ch:estadodaarte}

O estado da arte apresentado neste capítulo tem como objetivo contextualizar o desenvolvimento de \gls{usv} no panorama atual da robótica marítima. São discutidas as principais soluções de propulsão, desde abordagens baseadas em energias renováveis até plataformas motorizadas de alto desempenho, destacando as respetivas vantagens, limitações e áreas de aplicação. Adicionalmente, é feita uma ponte com o trabalho realizado durante a licenciatura, o qual serviu de base conceptual e experimental para o presente \gls{tfm}. Esta análise fornece o enquadramento necessário para compreender a evolução tecnológica nesta área e justifica as opções adotadas no desenvolvimento do protótipo proposto.

\section{Trabalho Relacionado} 
\label{sec:trabalhorelacionado}

O desenvolvimento de \gls{usv} tem atraído crescente atenção devido à sua relevância em operações de monitorização, recolha de dados e exploração de ambientes marítimos. Estas plataformas permitem realizar missões de forma segura e eficiente em áreas de difícil acesso ou potencialmente perigosas para embarcações tripuladas, constituindo uma solução cada vez mais valorizada em contextos científicos, ambientais e de defesa.  

Entre as alternativas de propulsão destacam-se soluções que dispensam motores convencionais. O \emph{Wave Glider}, desenvolvido pela Liquid Robotics \cite{wave-glider}, utiliza a energia das ondas para se deslocar de forma contínua e passiva, sendo particularmente adequado para missões de longa duração com baixo consumo energético. A Figura~\ref{fig:wave-glider} ilustra o princípio de funcionamento do mecanismo de propulsão por ondas (Figura \ref{fig:wave-glider} (A)) e a estrutura de superfície (Figura \ref{fig:wave-glider} (B)) responsável por garantir flutuabilidade e suporte para a instalação de sensores e antenas.

\begin{figure}[H]
    \centering
    \begin{minipage}{0.45\linewidth}
        \centering
        \includegraphics[width=\linewidth]{figuras/wave-glider.jpg}
        \caption*{(A) Mecanismo de propulsão por ondas}
    \end{minipage}
    \hfill
    \begin{minipage}{0.45\linewidth}
        \centering
        \includegraphics[width=\linewidth]{figuras/wave-glider2.png}
        \caption*{(B) Estrutura da superfície}
    \end{minipage}
    \caption[\gls{usv} Liquid Robotics Wave Glider]{\gls{usv} Liquid Robotics Wave Glider \cite{wave-glider}}
    \label{fig:wave-glider}
\end{figure}

Outra abordagem alternativa é o \emph{Saildrone} \cite{saildrone}, representado na Figura \ref{fig:saildrone}, que recorre à propulsão à vela. 

\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\linewidth]{saildrone-usv.jpg}
    \caption[\gls{usv} Saildrone]{\gls{usv} Saildrone \cite{saildrone}}
    \label{fig:saildrone}
\end{figure}

Este tipo de \gls{usv} tem sido amplamente utilizado em campanhas de monitorização oceânica de larga escala, tirando partido da elevada autonomia oferecida pela energia eólica.  

Apesar das vantagens associadas à utilização de fontes renováveis, as \gls{usv} motorizadas oferecem maior controlo de navegação, velocidade e manobrabilidade, características essenciais em ambientes de correntes variáveis ou condições marítimas adversas. Um exemplo relevante é o SeaRobotics SR-Surveyor \cite{sr-surveyor-class}, ilustrado na Figura \ref{fig:sr-surveyor-carrinha}, amplamente utilizado em levantamentos hidrográficos e mapeamento subaquático, equipado com sonar multifeixe para operações precisas em ambientes costeiros e portuários.  

\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\linewidth]{figuras/sr-surveyorm18-truck-bed.jpg}
    \caption[SeaRobotics SR-Surveyor]{SeaRobotics SR-Surveyor \cite{sr-surveyor-class}}
    \label{fig:sr-surveyor-carrinha}
\end{figure}

A Figura \ref{fig:c-cat-3-asv} apresenta outro exemplo, o \gls{asv} (subclasse de \gls{usv}) Global's C-Cat 3 \cite{c-cat-3-asv}.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\linewidth]{figuras/c-cat3.jpeg}
    \caption[\gls{asv} Global C-Cat 3]{\gls{asv} Global C-Cat 3 \cite{c-cat-3-asv}}
    \label{fig:c-cat-3-asv}
\end{figure}

O C-Cat~3 \cite{c-cat-3-asv} é um \gls{usv} de dimensões compactas, propulsionado por motor elétrico, projetado para operar em águas pouco profundas. A sua conceção modular e elevada manobrabilidade tornam-no particularmente adequado para missões de monitorização ambiental, levantamentos batimétricos e outras aplicações científicas ou de reconhecimento costeiro.

Também em Portugal têm surgido iniciativas relevantes, como o projeto Sea2Future \cite{sea2future,sea2future2}, desenvolvido pela Escola Superior Náutica Infante D. Henrique, que, conforme ilustrado na Figura \ref{fig:sea2future}, explora o potencial dos \gls{usv} motorizados em cenários de resgate e recolha de dados ambientais.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\linewidth]{figuras/sea2future.jpg}
    \caption[\gls{usv} do projeto Sea2Future]{\gls{usv} do projeto Sea2Future \cite{sea2future,sea2future2}}
    \label{fig:sea2future}
\end{figure}

Embora tecnologicamente sofisticadas, estas plataformas comerciais apresentam custos elevados, frequentemente superiores a 300\,000\,€, podendo ultrapassar meio milhão de euros dependendo do conjunto de sensores e da robustez estrutural requerida. A Tabela~\ref{tab:comparacao-usv-mini} resume as principais características das plataformas referidas, comparando o tipo de propulsão, o custo aproximado e as aplicações típicas.  

\begin{table}[H]
    \centering
    \renewcommand{\arraystretch}{1.15}
    \caption{Comparação de diferentes \gls{usv}}
    \label{tab:comparacao-usv-mini}
    \begin{tabular}{cccc}
        \toprule
        \textbf{Plataforma} & \textbf{Propulsão} & \textbf{Custo} & \textbf{Aplicações} \\ 
        \midrule
        Wave Glider & Ondas/Solar & 300\,000 € & Monitorização oceânica \\
        Saildrone & Vela/Solar & 500\,000 € & Monitorização oceânica \\
        SR-Surveyor & Elétrico & 500\,000 € & Hidrografia \\
        C-Cat 3 & Elétrico & 350\,000 € & Monitorização costeira \\
        Sea2Future & Elétrico & n/d & Dados ambientais \\
        Plataforma desenvolvida & Elétrico & 200 € & Demonstração funcional \\ 
        \bottomrule
    \end{tabular}
\end{table}

Nesta tabela, observa-se a diversidade de abordagens tecnológicas, desde soluções passivas que exploram energias renováveis (como o Wave Glider e o Saildrone) até veículos motorizados de elevado desempenho (SR-Surveyor, C-Cat 3, e Sea2Future). O último item refere-se ao \gls{usv} desenvolvido neste \gls{tfm}, cuja principal característica é o seu design modular, de baixo custo e com uma arquitetura de controlo versátil.  

Nos últimos anos, os avanços tecnológicos em sistemas de controlo e sensorização têm impulsionado o desenvolvimento de \gls{usv} mais acessíveis e modulares. Destaca-se o controlo independente de múltiplos motores, aliado à integração de sensores como \gls{gps}, \gls{imu} e sensores ambientais, que tornam estas plataformas cada vez mais autossuficientes e precisas, abrindo caminho para aplicações em larga escala em monitorização costeira, exploração científica e operações de segurança.

\section{\emph{Background}} 
\label{sec:background}

Durante o percurso académico da licenciatura, foi desenvolvido um projeto descrito em \cite{didactic-robot-thesis}, cujo objetivo principal consistiu na criação de um ambiente didático orientado para a exploração de conceitos fundamentais em robótica, como o controlo de movimento, a comunicação sem fios e a programação em C e Java. O resultado foi um protótipo funcional de robô didático, concebido como uma ferramenta simples mas completa do ponto de vista pedagógico.  

A arquitetura, apresentada em \cite{didactic-robot-thesis} e ilustrada na Figura~\ref{fig:arquitetura-robo-didatico}, foi concebida segundo uma abordagem modular e expansível, integrando até quatro motores, módulos de comunicação e sensores. Esta estrutura permitia a substituição ou expansão de componentes de forma rápida e segura, promovendo a reutilização e a experimentação prática.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\linewidth]{figuras/arquitetura_robo_didatico.png}
    \caption[Arquitetura do Robô Didático]{Arquitetura do Robô Didático \cite{didactic-robot-thesis}}
    \label{fig:arquitetura-robo-didatico}
\end{figure}

O protótipo físico, apresentado na Figura~\ref{fig:estrutura-robo-didatico}, demonstrou em ambiente real a integração de motores, sensores e módulos de comunicação, confirmando a eficácia da abordagem modular e a sua aplicabilidade em contextos didáticos e experimentais.  

\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\linewidth]{figuras/robot-didatico.jpg}
    \caption[Estrutura final do Robô Didático]{Estrutura final do Robô Didático \cite{didactic-robot-thesis}}
    \label{fig:estrutura-robo-didatico}
\end{figure}

O presente \gls{tfm} surge como uma evolução natural desse trabalho, aplicando os princípios de modularidade e escalabilidade ao desenvolvimento de um \gls{usv} com capacidade de navegação autónoma. Mantendo o barramento \gls{i2c} como base de comunicação, os motores \gls{dc} foram substituídos por propulsores \emph{brushless} controlados por \gls{esc}, mais adequados ao ambiente marítimo. Do mesmo modo, os encoders e sonares do robô terrestre foram substituídos por um \gls{imu} e um \gls{gps}, sensores capazes de medir orientação e posição absolutas, essenciais para corrigir erros provocados por correntes, vento e ondulação.  

Em paralelo, este trabalho relaciona-se também com o projeto desenvolvido pela \gls{enidh} em \cite{catamara-telecomandado}, que utilizou o mesmo catamarã como plataforma experimental. Apesar de funcional, essa versão apresentava limitações significativas ao nível da integração eletrónica e do controlo. O sistema baseia-se em duas placas Arduino (Mega e Nano) interligadas, com cablagem extensa e propensa a falhas, conforme ilustrado na Figura~\ref{fig:catamara-fios}.  

\begin{figure}[H]
    \centering
    \includegraphics[height=1.0\linewidth, angle=90]{figuras/catamara-fios.jpeg}
    \caption[Arquitetura eletrónica do catamarã atual]{Arquitetura eletrónica do catamarã desenvolvido em \cite{catamara-telecomandado}, evidenciando o emaranhado de fios utilizado para interligar os diversos módulos.}
    \label{fig:catamara-fios}
\end{figure}

Essa arquitetura rígida, dependente de comunicação por rádio frequência tradicional, limita o alcance e a escalabilidade do sistema. Em contraste, a solução proposta neste \gls{tfm} recorre a um único microcontrolador \gls{esp32}, que integra num só módulo o processamento e comunicação. A comunicação \gls{lora} substitui a rádio frequência, aumentando o alcance e a fiabilidade, enquanto a introdução de uma \gls{pcb} dedicada elimina o emaranhado de fios, reduzindo falhas e facilitando a replicação do sistema.

A principal diferença conceptual entre os dois sistemas reside na flexibilidade e adaptabilidade da arquitetura. O sistema anterior foi concebido especificamente para o catamarã, com ligações ponto a ponto e dependências fixas entre módulos. Já o sistema desenvolvido neste \gls{tfm} adota uma estrutura modular e reconfigurável, capaz de ser aplicada em diferentes plataformas marítimas ou terrestres com modificações mínimas de \emph{hardware} e \emph{software}.  

Em síntese, este trabalho consolida um percurso evolutivo iniciado com o robô didático e amadurecido com a análise crítica do catamarã existente. O resultado é uma plataforma ciberfísica integrada, escalável e versátil, que alia a robustez experimental à capacidade de adaptação a novos contextos de investigação e desenvolvimento em robótica marítima.
